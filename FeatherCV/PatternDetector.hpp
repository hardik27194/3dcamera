...
public:
    ...
    */
    ...
    bool findKeyPoints(const cv::Mat& image, std::vector<cv::KeyPoint>& keyPoints, cv::Mat& descriptors);
    
    
    bool findMatches(const cv::Mat& image,std::vector<cv::KeyPoint>& points, std::vector<cv::DMatch>& matches);
