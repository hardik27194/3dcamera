...
bool PatternDetector::findMatches(const cv::Mat &image,std::vector<cv::KeyPoint>& points, std::vector<cv::DMatch> &matches)
{
    cv::Mat grayImage;
    getGray(image, grayImage);
    cv::Mat descriptors;
    //std::vector<cv::KeyPoint> points;
    m_detector->detect(grayImage, points);
    std::cout << "detected key point:" << points.size() << std::endl;
    if(points.empty()){
        return false;
    }
    //cv::Mat descriptors;
    m_extractor->compute(grayImage, points, descriptors);
    std::cout << "after extraction:" << points.size() << ",descriptor:" << descriptors.cols << "," << descriptors.rows << std::endl;
    if(points.empty()){
        return false;
    }
    //std::vector<cv::DMatch>   matches;
    getMatches(descriptors, matches);
    if(matches.empty()){
        return false;
    }
    return true;
...
bool  PatternDetector::findKeyPoints(const cv::Mat& image, std::vector<cv::KeyPoint>& points, cv::Mat& descriptors)
{
    ...
    /**
    for(std::vector<cv::KeyPoint>::iterator ite = points.begin(); ite != points.end(); ++ite){
        EZKeyPoint pt;
        pt.position = ite->pt;
        pt.keyPoint = *ite;
        //pt.descriptors = descriptors;
...
void PatternDetector::train(const cv::Mat& descriptors)
{
    m_matcher->clear();
    std::vector<cv::Mat> desc(1);
    desc[0] = descriptors.clone();
    m_matcher->add(desc);
 ...
 void PatternDetector::train(const std::vector<cv::Mat>& descriptors)
{
    m_matcher->clear();
    std::vector<cv::Mat> desc(descriptors.size());
    for (int i = 0; i < descriptors.size(); ++i) {
        
        desc[i] = descriptors[i].clone();
        //m_matcher->add(desc);
    }
    m_matcher->add(desc);
    m_matcher->train();
